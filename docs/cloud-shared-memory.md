...
Choose **New note** (home or editor), press **Cmd/Ctrl+N** when delivered by your browser, or use **Cmd/Ctrl+Shift+Enter**. Browsers may reserve Cmd/Ctrl+N for a new window; the button always remains available. Creation saves a separate note immediately with a browser-local `YYYY-MM-DD HH:mm:ss.SSS` title/date heading, then focuses the body after that heading. No title or tag form is required. Repeated key events and double clicks are suppressed; **Retry new note** reuses the same request/key after a lost response while the tab remains open. Failed saves block creation from the current draft. Blank remote `note` creation is supported; findings, decisions, preferences, wrapups, and local CLI writes still require content.
