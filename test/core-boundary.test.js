...
test("core boundary exports backend-agnostic memory APIs without importing CLI", async () => {
  ...
  assert.equal(typeof core.renderMarkdownDocument, "function");
  ...
  assert.equal(core.rememberMemory, undefined, "core must not expose local write workflows");
  assert.equal(core.writeSessionWrapup, undefined, "core must not expose local wrapup workflows");
  assert.equal(core.writeRemoteMemoryNote, undefined, "core must not expose remote write workflows");
